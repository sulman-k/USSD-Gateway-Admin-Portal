...
exports.getDateRange = (startDate, endDate, defaultDate = true) => {
    ...
    if (defaultDate) {
        obj = {
            "$gt": new Date(moment(moment().subtract(config.DEFAULT_DATE_RANGE.difference, config.DEFAULT_DATE_RANGE.unit).format()).startOf("month").format()),
...
exports.getDateTimeList = (startDate, endDate, time_unit) => {
    if (time_unit === 'hour')
        ...
        return this.getMonthNamesByDate(startDate, endDate, time_unit);
};
