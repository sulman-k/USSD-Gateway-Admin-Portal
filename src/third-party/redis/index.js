...
exports.hashMapSet = async (key, field, val) => {
    logger.info("[hashMapSet][key][field][val]", `${key} ${field} ${JSON.stringify(val)}`);
...
exports.getRecordKey = async (obj) => {
    ...
    let data = await this.getSortedSetMembers(`${config.REDIS.USER_IDENTIFIER}${obj.uuid}`);

    logger.info("[getSortedSetMembers][callback][data]", `${JSON.stringify(data)}`);
    if (data.success) {
...
exports.dataFROMRTOM = async (records) => {
    ...
    const finalRecords = records.map((record) => {
        ...
        record = _.omit(record, ["error_code", "error_message", "id", "user", "model", "OS", "device", "brand", "manufacturer", "bootloader",
            "display", "host", "hardware", "board", "imei", "operator_name", "country", "sim_serial_number"]);
        return record;
    });
...
exports.dataDumpScheduler = async (pattern) => {
    ...
    let data = await scan_async(0, "MATCH", pattern, "COUNT", config.REDIS.SCAN_RECORDS_COUNT);
