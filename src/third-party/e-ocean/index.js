...
exports.getToPhone = async (data) => {
    ...
    return {
        success: true,
        data: { phoneno: body.data.caller ? body.data.caller : config.EOCEAN.TO_CALL.PHONE_NO }
    };
};
...
exports.getFromPhone = async (data) => {
    ...
    const obj = {};
    obj.method = () => {
        if (obj.response.success) logger.info("[getFromPhone][Success][Response]", obj.response);
    ...
    try {
        ...
        const url = `${config.EOCEAN.FROM_CALL.END_POINT}?username=${config.EOCEAN.FROM_CALL.QUERY_PARAMS.username}&password=${config.EOCEAN.FROM_CALL.QUERY_PARAMS.password}&number=${encodeURIComponent(cellNum)}`;
        const body = await getRequestHandler(config.EOCEAN.FROM_CALL.SERVER_URL, url);
        if (body && body.data && !body.data.success) {
            throw new Error("Server refused to provide phone number.");
        }
        obj.response = {
...
exports.smsFromServer = async (data) => {
    ...
    const obj = {};
    try {
        const short_code = await numbersPoolController.fromSMSCode(data.dialing_code);
        ...
        query_params["messagedata"] = `<#> Your VerifApp code is: ${data.code} \n${data.hash}`;
        const url = `${config.EOCEAN.FROM_SMS.END_POINT}?${queryString.encode(query_params)}`;
        const body = await getRequestHandler(config.EOCEAN.FROM_SMS.SERVER_URL, url);
...
exports.getToShortCode = async (data) => {
    ...
    logger.info("[getToShortCode][Success][Response]", JSON.stringify(short_code.response));
    return {
        success: true,
...
exports.whatsappSmsServer = async (data) => {
    ...
    try {
        ...
        let response = await postRequestHandler(config.WHATSAPP_BUSINESS.BASEURL, config.WHATSAPP_BUSINESS.SEND_MESSAGE_URL, headers, body)
        if (response.status === 202) {
            logger.info("[whatsappSmsServer][successResponse]", response.data);
            return { success: true, data: response }
        } else if (response.status === 401) { // Auth error
            // generate token again and send message
            await this.generateWBToken()
            JWT = await this.getWBToken();
            headers['Authorization'] = JWT.trim();

            response = await postRequestHandler(config.WHATSAPP_BUSINESS.BASEURL, config.WHATSAPP_BUSINESS.SEND_MESSAGE_URL, headers, body)
...
const postRequestHandler = async (base_url, url, headers, body) => {
    ...
    return await axios({
        ...
        if (e.response.status === 401) { // Auth error
            // generate token again and send message
            await this.generateWBToken()
            JWT = await this.getWBToken();
            headers['Authorization'] = JWT.trim();
...
exports.generateWBToken = async (req, res) => {
    try {
        ...
        let response = await postRequestHandler(config.WHATSAPP_BUSINESS.BASEURL, config.WHATSAPP_BUSINESS.GET_TOKEN_URL, {}, body)
        if (response.status === 201) {
            ...
            return new Promise(resolve => {
                fs.writeFile("src/config/JWTAuth.txt", JWT, (err) => {
                    ...
                    logger.info("[generateWBToken][success]", `Whatsapp SMS Token generated successfully.`);
                    resolve("ok")
                })
            })
        } else {
            logger.error("[generateWBToken][Error][Response]", "Error while retrieving Token");
        }
    } catch (error) {
